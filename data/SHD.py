...
class SHD(LightningDataModule):
    def __init__(self, data_dir: str = "../data/shd", batch_size: int = 32, num_workers: int = 4, n_time_bins: int = 40, seed: int = 42):
        super().__init__()
        ...
        self.transform = transforms.Compose([
            transforms.ToFrame(sensor_size=tonic.datasets.SHD.sensor_size, n_time_bins=n_time_bins),
            transforms.NumpyAsType('float32'),
        ])
        self.target_transform = None

    def prepare_data(self):
        tonic.datasets.SHD(save_to=self.data_dir, train=True)
        tonic.datasets.SHD(save_to=self.data_dir, train=False)

    def setup(self, stage):
        match stage:
            case 'fit':
                full_train_dataset = tonic.datasets.SHD(save_to=self.data_dir, train=True,
            ...
            case 'test':
                self.test_dataset = tonic.datasets.SHD(save_to=self.data_dir, train=False,
