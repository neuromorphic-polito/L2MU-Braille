...
from architecture.quantized.network_system.snn.backend_module.l2mu import L2MULeaky, L2MUSynaptic
...
class L2MUNet(nn.Module):
    ...
    def select_neuron_type(self, input_size, output_size, neuron_type, encoder, params):
        match neuron_type:
            case 'leaky':
                self.slmu = L2MULeaky(input_size=input_size, output=True, output_size=output_size, encoder=encoder,
                                 params=params)
            case 'synaptic':
                self.slmu = L2MUSynaptic(input_size=input_size, output=True, output_size=output_size, encoder=encoder,
                                 params=params)
    ...
    def forward(self, input):
        ...
        x = self.dequant(torch.stack(spk_output_list, dim=0))
