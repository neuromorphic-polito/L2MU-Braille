...
class SinglePopEncoder(nn.Module):
    ...
    def __init__(self, input_size, params, *args, **kwargs):
        ...
        for i in range(self.input_size):
            self.mem_pop_encoder.append(None)
            self.syn_pop_encoder.append(None)
        ...
        for i in range(self.input_size):
            ...
            self.spk_pop_encoder_list.append(
                Synaptic(beta=self.neurons_hidden_encored_conf[i][0], threshold=self.neurons_hidden_encored_conf[i][1],
                         alpha=self.neurons_hidden_encored_conf[i][2]))

        self.pop_encoder_list = nn.ModuleList(self.pop_encoder_list)
        self.spk_pop_encoder_list = nn.ModuleList(self.spk_pop_encoder_list)

    def forward(self, input_):

        spk_pop_encoded_signal = []
        for i, (encoder_i, spk_pop_encoder_i) in enumerate(
                zip(self.pop_encoder_list, self.spk_pop_encoder_list)):
            # Use 'i' to slice the input
            pop_encoded_feature_i = encoder_i(input_[:, i].unsqueeze(1))

            # Apply the spike encoder
            spk_pop_encoded_feature_i, self.syn_pop_encoder[i], self.mem_pop_encoder[i] = spk_pop_encoder_i(pop_encoded_feature_i,
                                                                                            self.syn_pop_encoder[i], self.mem_pop_encoder[i])
    ...
    def init_pop_encoder(self):
        for i, spk_pop_encoder_list_i in enumerate(self.spk_pop_encoder_list):
            self.syn_pop_encoder[i], self.mem_pop_encoder[i] = spk_pop_encoder_list_i.init_synaptic()
