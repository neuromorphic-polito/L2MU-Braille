...
class StackedPopEncoder(nn.Module):
    ...
    def forward(self, input_):
        ...
        for i, (encoder_i, spk_hidden_encoder_i) in enumerate(
            ...
            spk_hidden_encoded_feature_i, self.syn_hidden_encoder[i], self.mem_hidden_encoder[i] = spk_hidden_encoder_i(hidden_encoded_feature_i,
                                                                                         self.syn_hidden_encoder[i], self.mem_hidden_encoder[i])
        ...
        spk_encoder_signal, self.syn_encoder, self.mem_encoder = self.spk_encoder(encoder_signal, self.syn_encoder, self.mem_encoder)
        decoder_signal = self.decoder(spk_encoder_signal)

        spk_decoder_signal, self.syn_decoder, self.mem_decoder = self.spk_decoder(decoder_signal, self.syn_decoder, self.mem_decoder)
    ...
    def init_pop_encoder(self):
        for i, spk_hidden_encoder_list_i in enumerate(self.spk_hidden_encoder_list):
            self.syn_hidden_encoder[i], self.mem_hidden_encoder[i] = spk_hidden_encoder_list_i.init_synaptic()
