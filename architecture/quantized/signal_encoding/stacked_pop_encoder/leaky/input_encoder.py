import torch
from spike_core_pq.snntorchqt.snntorch import Leaky
...
class StackedPopEncoder(nn.Module):
    ...
    def __init__(self, input_size, params, *args, **kwargs):
        ...
        self.output_size = params['output_transformer_size']
        ...
        for i in range(self.input_size):
            ...
            self.spk_hidden_encoder_list.append(
                Leaky(beta=self.neurons_hidden_encored_conf[i][0], threshold=self.neurons_hidden_encored_conf[i][1]))
        ...
        self.spk_encoder = Leaky(beta=params['beta_spk_encoder'], threshold=params['threshold_spk_encoder'])

        self.decoder = nn.Linear(self.encoding_size, self.output_size)
        self.spk_decoder = Leaky(beta=params['beta_spk_decoder'], threshold=params['threshold_spk_decoder'])

    def forward(self, input_):

        spk_hidden_encoded_signal = []
        for i, (encoder_i, spk_hidden_encoder_i) in enumerate(
                zip(self.hidden_encoder_list, self.spk_hidden_encoder_list)):
            # Use 'i' to slice the input
            hidden_encoded_feature_i = encoder_i(input_[:, i].unsqueeze(1))

            # Apply the spike encoder
            spk_hidden_encoded_feature_i, self.mem_hidden_encoder[i] = spk_hidden_encoder_i(hidden_encoded_feature_i,
                                                                                         self.mem_hidden_encoder[i])

            # Store the updated memory state back if necessary
            # Note: This will not update the original list. See the note below for handling this.

            # Append the spike-encoded feature to the list
        ...
        spk_encoder_signal, self.mem_encoder = self.spk_encoder(encoder_signal, self.mem_encoder)
        decoder_signal = self.decoder(spk_encoder_signal)

        spk_decoder_signal, self.mem_decoder = self.spk_decoder(decoder_signal, self.mem_decoder)

        return spk_decoder_signal

    def init_pop_encoder(self):
        for i, spk_hidden_encoder_list_i in enumerate(self.spk_hidden_encoder_list):
            self.mem_hidden_encoder[i] = spk_hidden_encoder_list_i.init_leaky()
        self.mem_encoder = self.spk_encoder.init_leaky()
        self.mem_decoder = self.spk_decoder.init_leaky()
