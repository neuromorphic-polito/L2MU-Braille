...
class StackedPopEncoder(nn.Module):
    ...
    def forward(self, input_):
        ...
        for i in range(self.input_size):
            hidden_encoded_feature_i = self.hidden_encoder_list[i](input_[:, i].unsqueeze(1))
            spk_hidden_encoded_feature_i, self.mem_hidden_encoder[i] = self.spk_hidden_encoder_list[i](
                hidden_encoded_feature_i, self.mem_hidden_encoder[i])
    ...
    def init_pop_encoder(self):
        for i in range(self.input_size):
            self.mem_hidden_encoder[i] = self.spk_hidden_encoder_list[i].init_leaky()
