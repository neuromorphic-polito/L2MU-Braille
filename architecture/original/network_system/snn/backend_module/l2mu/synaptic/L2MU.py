...
from architecture.original.signal_encoding.stacked_pop_encoder import StackedPopEncoderSynaptic
from architecture.utils import initialize_states
from architecture.original.network_system.bare_lmu_network.bare_LMU import BareLMU
...
class L2MU(BareLMU):
    ...
    def __init__(
        ...
        self.spk_u = Synaptic(beta=params['beta_spk_u'], threshold=params['threshold_spk_u'],
                              alpha=params['alpha_spk_u'])
        self.spk_h = Synaptic(beta=params['beta_spk_h'], threshold=params['threshold_spk_h'],
                              alpha=params['alpha_spk_h'])
        self.spk_m = Synaptic(beta=params['beta_spk_m'], threshold=params['threshold_spk_m'],
                              alpha=params['alpha_spk_m'])

        self.syn_m, self.mem_m = None, None
        self.syn_h, self.mem_h = None, None
        self.syn_u, self.mem_u = None, None

        if self.output:
            self.spk_output = Synaptic(beta=params['beta_spk_output'], threshold=params['threshold_spk_output'],
                                       alpha=params['alpha_spk_output'])
    ...
    def init_l2mu(self):
        self.syn_m, self.mem_m = self.spk_m.init_synaptic()
        self.syn_h, self.mem_h = self.spk_h.init_synaptic()
        self.syn_u, self.mem_u = self.spk_u.init_synaptic()
    ...
    def forward(self, input_, spk_hidden, spk_memory):
        ...
        if self.input_encoder is None:
            ...
            spk_input = self.input_encoder(input_)
        ...
        spk_u, self.syn_u, self.mem_u = self.spk_u(curr_u, self.syn_u, self.mem_u)
        ...
        spk_memory, self.syn_m, self.mem_m = self.spk_m(curr_m, self.syn_m, self.mem_m)
        ...
        spk_hidden, self.syn_h, self.mem_h = self.spk_h(curr_h, self.syn_h, self.mem_h)  # [batch_size, hidden_size]

        # Output
        if self.output:
            curr_output = self.output_transformation(spk_hidden)
            spk_output, self.syn_output, self.mem_output = self.spk_output(curr_output, self.syn_output,
                                                                           self.mem_output)
