...
from architecture.original.signal_encoding.single_pop_encoder import SinglePopEncoderLeaky
from architecture.original.signal_encoding.stacked_pop_encoder import StackedPopEncoderLeaky
...
class SRNN(nn.Module):
    ...
    def __init__(self, input_size, output_size, params, encoder='plain'):
        ...
        self.spk_hidden = snn.RLeaky(beta=params['beta_hidden'], all_to_all=True, threshold=params['threshold_hidden'],
                                     linear_features=int(params['hidden_size']))

        # Output layer
        self.fc2 = nn.Linear(self.hidden_size, output_size)
        self.spk_output = snn.Leaky(beta=params['beta_output'], threshold=params['threshold_output'])

        self.spk_hn, self.mem_hidden = None, None
    ...
    def init_network(self):
        ...
        self.spk_hn, self.mem_hidden = self.spk_hidden.init_rleaky()
    ...
    def forward(self, input_):
        ...
        self.spk_hn, self.mem_hidden = self.spk_hidden(curr1, self.spk_hn, self.mem_hidden)

        # Output layer
        cur2 = self.fc2(self.spk_hn)
        spk2, self.mem_output = self.spk_output(cur2, self.mem_output)
